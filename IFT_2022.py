...
def run():
    ...
    expInfo = {'participant': '', 'startorder': ['Run1', 'Run2', 'Run3', 'Run4']}
    ...
    showWelc(win)
    newInstruct(win, c.INST1)
    newInstruct(win, c.INST2)
    newInstruct(win, c.INST3)
    showInstructCue(win, c.INST4, "stimuli/TUP.BMP", "25¢")
    newInstruct(win, c.INST5)
    ...
    with open(f"{save_filename}.csv", 'w', newline='') as csvfile:
        ...
        for step in range(4):
            ...
            for i, row in df.iterrows():
                ...
                #cuejitter = random.uniform(1.5,10.0)
